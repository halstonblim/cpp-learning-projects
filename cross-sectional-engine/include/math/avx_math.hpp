...
// Aligned version - requires 32-byte aligned pointers
...
// Aligned version - requires 32-byte aligned data pointer
...
inline void avx_zscore(const float* data, size_t size, float mean, float std_dev, float* output) {
    ...
    for (; i + 8 <= size; i += 8) {
        ...
        _mm256_store_ps(&output[i], zscore_vec);
    }
    for (; i < size; ++i) {
...
// ============================================================================
// Unaligned versions - for data that may not be 32-byte aligned
// (e.g., sub-array access at arbitrary offsets)
// ============================================================================

[[nodiscard]] inline float avx_mean_unaligned(const float* data, size_t size) {
    if (size == 0) return 0.0f;
...
[[nodiscard]] inline float avx_std_dev_unaligned(const float* data, size_t size, float mean) {
    if (size == 0) return 0.0f;
...
inline void avx_zscore_unaligned(const float* data, size_t size, float mean, float std_dev, float* output) {
    if (size == 0) return;
